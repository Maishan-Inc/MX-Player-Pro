...
var N = class {
	...
	async init() {
		...
		let t = e.size ?? this.loader.totalSize ?? C[C.length - 1], n = /* @__PURE__ */ new Uint8Array(), r = null, i = !1;
		for (let e of C) {
...
var V = null, H = !1, U = 0, W = Promise.resolve();
function G(e) {
	if (e.type === "packets") {
...
self.onmessage = (e) => {
	let t = e.data;
	t.type === "init" ? U = 0 : "epoch" in t && t.epoch > U && (U = t.epoch), W = W.then(() => K(t)).catch(() => void 0);
};
async function K(e) {
	...
	try {
		...
		if (!V || !H) {
			(e.type === "next" || e.type === "seek" || e.type === "select-track") && G({
				type: "packets",
				packets: [],
				epoch: t
			});
		...
		if (t < U && (e.type === "next" || e.type === "seek")) return;
		if (e.type === "seek") G({
			type: "progress",
			phase: "定位关键帧",
			value: .2
			...
			type: "packets",
			packets: await V.packetsFor(e.time),
			epoch: t
		});
		else if (e.type === "next") {
			let e = await V.next();
		...
		} else e.type === "select-track" ? (V.select(e.kind, e.trackId), e.kind === "subtitle" ? G({
			type: "packets",
			packets: [],
			epoch: t
		}) : G({
