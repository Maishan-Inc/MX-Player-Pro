...
var A = class {
	...
	async next() {
		...
		return this.cursor >= this.segmentEnd && (this.atEnd = !0), e.length ? e.sort((e, t) => e.timestamp - t.timestamp) : [];
	}
	select(e, t) {
...
var R = null, z = !1, B = 0, V = Promise.resolve();
function H(e) {
	if (e.type === "packets") {
...
self.onmessage = (e) => {
	let t = e.data;
	t.type === "init" ? B = 0 : "epoch" in t && t.epoch > B && (B = t.epoch), V = V.then(() => U(t)).catch(() => void 0);
};
async function U(e) {
	...
	try {
		...
		if (!R || !z) {
			(e.type === "next" || e.type === "seek" || e.type === "select-track") && H({
				type: "packets",
				packets: [],
				epoch: t
			});
		...
		if (t < B && (e.type === "next" || e.type === "seek")) return;
		if (e.type === "seek") H({
			type: "progress",
			phase: "定位关键帧",
			value: .2
			...
			type: "packets",
			packets: await R.packetsFor(e.time),
			epoch: t
		});
		else if (e.type === "next") {
			let e = await R.next();
		...
		} else e.type === "select-track" ? (R.select(e.kind, e.trackId), e.kind === "subtitle" ? H({
			type: "packets",
			packets: [],
			epoch: t
		}) : H({
