...
var f = (...e) => e.filter((e, t, n) => !!e && e.trim() !== "" && n.indexOf(e) === t).join(" ").trim(), p = (e) => e.replace(/([a-z0-9])([A-Z])/g, "$1-$2").toLowerCase(), ee = (e) => e.replace(/^([A-Z])|[\s-_]+(\w)/g, (e, t, n) => n ? n.toUpperCase() : t.toLowerCase()), m = (e) => {
	let t = ee(e);
	...
	return !1;
}, ne = t({}), re = () => i(ne), ie = r(({ color: e, size: t, strokeWidth: r, absoluteStrokeWidth: i, className: a = "", children: o, iconNode: s, ...c }, l) => {
	let { size: u = 24, strokeWidth: d = 2, absoluteStrokeWidth: p = !1, color: ee = "currentColor", className: m = "" } = re() ?? {}, ne = i ?? p ? Number(r ?? d) * 24 / Number(t ?? u) : r ?? d;
	return n("svg", {
		ref: l,
		...h,
		width: t ?? u ?? h.width,
		...
		stroke: e ?? ee,
		strokeWidth: ne,
		className: f("lucide", m, a),
		...!o && !te(c) && { "aria-hidden": "true" },
		...c
	...
	let i = r(({ className: r, ...i }, a) => n(ie, {
		ref: a,
		iconNode: t,
		className: f(`lucide-${p(m(e))}`, `lucide-${e}`, r),
		...i
	}));
	return i.displayName = m(e), i;
}, ae = g("arrow-left", [["path", {
	d: "m12 19-7-7 7-7",
	key: "1l729n"
}], ["path", {
	d: "M19 12H5",
	...
	rx: "1",
	key: "1wsw3u"
}]]), pe = g("picture-in-picture-2", [["path", {
	...
	key: "10ikf1"
}]]), he = g("rectangle-horizontal", [["rect", {
	width: "20",
	height: "12",
	x: "2",
	y: "6",
...
function we(e) {
	let t = e.replace(y, "").trim();
	if (!t) return "";
	let n = t.match(/^Dialogue:\s*(.*)$/is), r = (n ? n[1] : t).split(","), i = n ? 9 : 8;
	return Ce(r.length > i ? r.slice(i).join(",") : t);
}
function Te(e, t) {
...
function De(e) {
	return e.kind === "subtitle" && b.has(e.codecId.toUpperCase());
}
function Oe(e) {
...
var x = [
	...
	max: 74,
	step: 1
}, T = "mx-player-pro:subtitle-style:";
function ke(e) {
	return (x.find((t) => t.id === e) || x[0]).stack;
}
function Ae(e) {
	return Number.isFinite(e) ? Math.round(Math.min(C.max, Math.max(C.min, e)) * 10) / 10 : S.scale;
}
function je(e) {
	return Number.isFinite(e) ? Math.round(Math.min(w.max, Math.max(w.min, e))) : S.offset;
}
function E(e) {
	return {
		font: x.some((t) => t.id === e?.font) ? e.font : S.font,
		scale: Ae(Number(e?.scale)),
		offset: je(Number(e?.offset))
	};
}
function D(e) {
...
function Ne(e, t) {
	try {
		localStorage.setItem(T + e, JSON.stringify(E(t)));
	} catch {}
}
var O = .004, k = class {
	items = [];
...
var N = class {
	...
	async configure(e, t) {
		...
		this.ctx = this.canvas.getContext("2d", {
			alpha: !1,
			desynchronized: !0
		}), this.audioClock = null, this.clock = new j(new A()), this.frames.clearFloor(), this.awaitingKeyframe = !0, this.endOfStream = !1, this.flushed = !1, this.stalled = !1, this.seekTarget = 0, this.packets.setActive("video", !!(e?.codec && e.width && e.height)), this.packets.setActive("audio", !!t?.codec);
		let r = !1, i = !1;
	...
	needsPackets(e = this.playing, t = !1, n = !1) {
		return !this.packets.isActive("video") && !this.packets.isActive("audio") ? !1 : Fe({
	...
	pumpDecoders() {
		let e = globalThis;
		for (; this.videoDecoder && e.EncodedVideoChunk && this.packets.pending("video") && Ie(this.decodePressure("video"));) {
		...
		for (; this.playing && this.audioDecoder && e.EncodedAudioChunk && this.packets.pending("audio") && M(this.decodePressure("audio"));) {
	...
	checkAudioLiveness() {
		!this.playing || !this.packets.isActive("audio") || this.packets.endOf("audio") === -Infinity && this.packets.endOf("video") !== -Infinity && (performance.now() - this.audioWaitSince < P || (this.packets.setActive("audio", !1), this.rebuildClockWithoutAudio(), this.onStatus({
	...
	rebuildClockWithoutAudio() {
		...
		this.audioClock = null, this.clock = new j(new A()), this.clock.reset(e), this.clock.setRate(this.playbackRate), this.playing && !this.stalled && this.clock.start();
	}
	startRenderLoop() {
	...
	acceptFrame(e) {
		this.frames.push(new F(e)), this.previewPending && (this.previewPending = !1, this.renderPreviewFrame());
	}
	onAudioData(e) {
...
function ze(e) {
	...
	try {
		...
		return new Worker("data:text/javascript;charset=utf-8," + encodeURIComponent(R), {
			type: "module",
			name: e?.name
		});
	}
...
function Ve(e, t, n = Worker) {
	return e ? new n(e, Be) : new t(Be);
}
//#endregion
...
var B = "1.2.0", Ue = 2048, We = "ABCabc123", V = 34, Ge = 4, Ke = {
	currentTime: 0,
	bufferedStart: 0,
	bufferedEnd: 0,
	bufferedAhead: 0,
	...
	let { source: r, label: i = "MX Player Pro", onExit: s, embedded: f = !1, autoplay: p = !1, initialVolume: ee = .85, initialMuted: m = !1, workerUrl: h, onNext: te, qualities: ne = [], selectedQuality: re = "auto", onQualityChange: ie, danmaku: g, className: se, style: ce } = t, _ = c(null), _e = c(null), v = c(null), y = c(null), b = c(0), x = c(!1), C = c(!1), w = c(!1), T = c(null), E = c(null), D = c(null), O = c({
		x: 0,
		y: 0
	}), k = c(!1), [A, Pe] = l(null), [j, Fe] = l(null), [Ie, M] = l(r ? "正在连接媒体…" : "等待媒体地址"), [N, P] = l(""), [F, I] = l(!1), [L, R] = l(m), [z, ze] = l(ot(ee)), [Be, Ve] = l(1), [B, V] = l(0), [Ge, qe] = l(), [st, ct] = l(), [lt, ut] = l(null), [dt, ft] = l(!1), [pt, mt] = l([]), [ht, U] = l(!1), [gt, _t] = l(!1), [vt, yt] = l("track"), [W, bt] = l(!1), [G, xt] = l(() => Me(at(r))), [St, Ct] = l(!0), [wt, Tt] = l(!1), [Et, Dt] = l(!1), [Ot, kt] = l({
		open: !1,
		x: 0,
		y: 0
	}), [At, jt] = l(!1), [Mt, Nt] = l(!1), [Pt, Ft] = l("等待 WebCodecs…"), [K, It] = l(Ke), [Lt, Rt] = l(g?.visible ?? !0), [zt, Bt] = l(0), q = c(!1), Vt = c(void 0), Ht = c(void 0), Ut = c(null), Wt = c(!1), Gt = c(/* @__PURE__ */ new Map()), Kt = c(/* @__PURE__ */ new Map()), qt = c(""), Jt = c(() => void 0), Yt = c(() => void 0), Xt = c(0), Zt = c(at(r)), Qt = c(!1), $t = c(() => void 0), J = c(t), en = c(null), tn = c(null);
	J.current = t;
	let nn = A?.tracks.filter((e) => e.kind === "video") || [], rn = A?.tracks.filter((e) => e.kind === "audio") || [], an = A?.tracks.filter((e) => e.kind === "subtitle") || [], on = an.filter(De), Y = A?.duration || 0, sn = on.find((e) => e.id === lt), X = gt || W;
	q.current = F, Vt.current = Ge, Ht.current = st, Ut.current = lt, Wt.current = dt, Xt.current = Y, Jt.current = un, Yt.current = cn, $t.current = $, o(n, () => ({
		play: pn,
		pause: mn,
		toggle: hn,
		seek: Z,
		setVolume: zn,
		setMuted: (e) => {
			R(e), y.current?.setVolume(e ? 0 : z);
		},
		setPlaybackRate: (e) => {
			let t = Math.max(.25, Math.min(4, e));
			Ve(t), y.current?.setPlaybackRate(t);
		},
		requestFullscreen: _n,
		requestPictureInPicture: vn,
		getState: () => ({
			ready: w.current,
			playing: q.current,
			currentTime: y.current?.currentTime ?? B,
			duration: Y,
			volume: z,
			muted: L,
			playbackRate: Be,
		...
		let e = _e.current;
		if (!e || !r) {
			Pe(null), Fe(null), M("等待媒体地址"), P(""), I(!1), V(0), It(Ke);
			return;
		}
		Pe(null), Fe(null), M("正在连接媒体…"), P(""), I(!1), q.current = !1, V(0), It(Ke), mt([]), qt.current = "";
		let t;
		try {
			t = He(h);
		} catch (e) {
			let t = Re(e instanceof Error ? e.message : String(e));
			P(t), M("Worker 创建失败"), J.current.onError?.({ message: t });
			return;
		}
		let n = new Le(e, (e) => {
			if (Ft(e.error ? Re(e.error) : `${e.videoReady ? "视频" : "视频不可用"}${e.audioReady ? " · 音频" : " · 音频不可用"}`), e.error && !/^DECODER_(?:ERROR|UNSUPPORTED)_AUDIO/i.test(e.error)) {
				let t = Re(e.error);
				P(t), J.current.onError?.({ message: t });
			}
		});
		v.current = t, y.current = n, b.current = 0, x.current = !1, C.current = !1, w.current = !1, t.onmessage = (e) => Jt.current(e.data), t.postMessage({
			type: "init",
			source: r
		});
		let i = window.setInterval(() => {
			...
			let t = e.stats();
			It(t);
			let n = Xt.current ? Math.min(t.currentTime, Xt.current) : t.currentTime;
			V(n), J.current.onTimeUpdate?.({
				currentTime: n,
				duration: Xt.current
			}), ln(n), Yt.current();
		}, 100);
		return () => {
			window.clearInterval(i), t.postMessage({ type: "close" }), t.terminate(), n.close(), v.current = null, y.current = null;
		};
	}, [
		r,
		h,
		...
		function e() {
			Tt(document.fullscreenElement === _.current);
		}
		function t(e) {
			let t = e.target;
			t && _.current?.contains(t) || (kt((e) => e.open ? {
				...e,
				open: !1
			} : e), U(!1), $t.current());
		}
		...
		T.current !== null && window.clearTimeout(T.current), E.current !== null && window.clearTimeout(E.current), D.current !== null && window.clearTimeout(D.current), tn.current?.getTracks().forEach((e) => e.stop()), tn.current = null, en.current?.remove(), en.current = null;
	}, []), a(() => {
		let e = at(r);
		Zt.current = e, xt(Me(e));
	}, [r]), a(() => {
		Ne(Zt.current, G);
	}, [G]);
	function cn() {
		!w.current || x.current || C.current || y.current?.needsPackets(q.current, C.current, x.current) && (x.current = !0, v.current?.postMessage({
			type: "next",
			epoch: b.current
		}));
	}
	function ln(e) {
		let t = Ut.current, n = Wt.current && t !== null ? Gt.current.get(t) : void 0, r = n ? Te(n, e).map((e) => e.text) : [], i = r.join(" ");
		i !== qt.current && (qt.current = i, mt(r));
	}
	function un(e) {
		if (e.type === "progress") {
			M(e.phase);
		...
		if (e.type === "error") {
			x.current = !1;
			let t = Re(e.message);
			P(t), M("读取失败"), J.current.onError?.({ message: t });
			return;
		}
		if (e.type === "metadata") {
			...
			}), Fe(e.probe), qe(n?.id), ct(r?.id), Vt.current = n?.id, Ht.current = r?.id, ut(null), Ut.current = null, ft(!1), Wt.current = !1, Gt.current = /* @__PURE__ */ new Map(), Kt.current = new Map(t.filter(De).map((e) => [e.id, Oe(e)])), qt.current = "", mt([]), M("轨道已识别"), w.current = !0, y.current?.configure(n, r), y.current?.setVolume(L ? 0 : z), J.current.onReady?.({
				tracks: t,
				duration: e.metadata.duration
			}), p && window.setTimeout(() => {
				!q.current && !X && hn();
		...
		if (e.type === "packets") {
			if (e.epoch < b.current) return;
			x.current = !1, e.packets.forEach((e) => dn(e)), e.packets.length && cn();
			return;
		}
		if (e.type === "eof") {
			if (e.epoch < b.current) return;
			x.current = !1, C.current = !0, y.current?.markEndOfStream(), M("已到达文件末端"), J.current.onEnded?.();
		}
	}
	function dn(e) {
		let t = Kt.current.get(e.trackId);
		if (t !== void 0) {
			let n = new TextDecoder().decode(e.data), r = t ? we(n) : Ce(n.trim());
			r && fn(e, r);
			return;
		}
		y.current?.enqueue(e, Vt.current, Ht.current);
	}
	function fn(e, t) {
		let n = e.timestamp / 1e6, r = n + (e.duration > 0 ? e.duration / 1e6 : 3), i = Gt.current.get(e.trackId) || [], a = i.length;
		...
		if (!(a > 0 && i[a - 1].start === n && i[a - 1].text === t)) {
			...
			i.splice(a, 0, {
				...
				text: t
			}), i.length > Ue && i.splice(0, i.length - Ue), Gt.current.set(e.trackId, i);
		}
	}
	function pn() {
		if (X || !w.current || !y.current || q.current) {
			Q(X);
			return;
		}
		I(!0), q.current = !0, y.current.play(), cn(), J.current.onPlay?.(), Q();
	}
	function mn() {
		!y.current || !q.current || (I(!1), q.current = !1, y.current.pause(), J.current.onPause?.(), Q());
	}
	function hn() {
		if (X) {
			Q(!0);
	...
	function Z(e) {
		let t = Math.max(0, Math.min(e, Y || e));
		V(t), b.current += 1, C.current = !1, x.current = !1, y.current?.seekTo(t), ln(t), v.current?.postMessage({
			type: "seek",
			time: t,
			epoch: b.current
		}), x.current = !0, Q();
	...
	function gn(e, t) {
		if (e === "subtitle") {
			ut(t), Ut.current = t, ft(t !== null), Wt.current = t !== null, $(), ln(y.current?.currentTime ?? B);
			return;
		}
		if (ct(t === null ? void 0 : t), Ht.current = t === null ? void 0 : t, t === null) return;
		let n = y.current?.currentTime ?? 0;
		b.current += 1, C.current = !1, y.current?.seekTo(n), v.current?.postMessage({
			type: "select-track",
			kind: e,
			trackId: t,
			time: n,
	...
	async function vn() {
		...
		let n = en.current;
		n || (n = document.createElement("video"), n.muted = !0, n.playsInline = !0, n.style.display = "none", document.body.appendChild(n), en.current = n), tn.current?.getTracks().forEach((e) => e.stop());
	...
	function yn() {
		let e = !Et;
		Dt(e), J.current.onTheaterChange?.(e);
	}
	function bn() {
		let e = !Lt;
		Rt(e), g?.onToggle?.(e);
	...
	function Q(e = !1) {
		Ct(!0), E.current !== null && window.clearTimeout(E.current), !(e || xn()) && (E.current = window.setTimeout(() => Ct(!1), 5e3));
	}
	function xn() {
		return Ot.open || At || Mt || ht || gt || W;
	}
	function Sn(e) {
		et(e.target) || (An(), T.current !== null && window.clearTimeout(T.current), T.current = window.setTimeout(() => {
			T.current = null, hn();
		}, 220));
	}
	function Cn(e) {
		et(e.target) || (T.current !== null && (window.clearTimeout(T.current), T.current = null), _n());
	}
	function wn(e) {
		let t = e.target;
		if ([
		...
		if ([
			...
			Q(), n === " " ? hn() : n === "arrowleft" ? Z(B - 5) : n === "arrowright" ? Z(B + 5) : n === "j" ? Z(B - 10) : n === "l" ? Z(B + 10) : n === "arrowup" ? zn(Math.min(1, z + .05)) : n === "arrowdown" ? zn(Math.max(0, z - .05)) : n === "m" ? Bn() : n === "f" && _n();
		}
	}
	function Tn(e) {
		if (et(e.target)) return;
		let t = e.touches[0];
		t && (O.current = {
			x: t.clientX,
			y: t.clientY
		}, k.current = !1, D.current !== null && window.clearTimeout(D.current), D.current = window.setTimeout(() => {
			k.current = !0, kn(O.current.x, O.current.y);
		}, 500));
	}
	function En(e) {
		let t = e.touches[0];
		!t || D.current === null || (Math.abs(t.clientX - O.current.x) > 10 || Math.abs(t.clientY - O.current.y) > 10) && (window.clearTimeout(D.current), D.current = null);
	}
	function Dn(e) {
		if (!et(e.target)) {
			if (e.preventDefault(), D.current !== null && (window.clearTimeout(D.current), D.current = null), k.current) {
				k.current = !1;
				return;
			}
			St ? Ct(!1) : Q();
	...
	function On(e) {
		e.preventDefault(), kn(e.clientX, e.clientY);
	}
	function kn(e, t) {
		let n = _.current;
		if (!n) return;
	...
	function Nn() {
		...
		Pn(), _t(!0), yt("track"), U(!1), Q(!0);
	}
	function Pn() {
		X || (Qt.current = !F), F && (I(!1), q.current = !1, y.current?.pause(), J.current.onPause?.());
	}
	function $() {
		!gt && !W || (_t(!1), bt(!1), Qt.current || (I(!0), q.current = !0, y.current?.play(), cn(), J.current.onPlay?.()), Q());
	}
	function Fn() {
		Pn(), U(!1), bt(!0), Q(!0);
	}
	...
	function Ln(e) {
		if (!_.current) return;
		e.stopPropagation();
		let t = _.current, n = e.clientY, r = G.offset, i = t.getBoundingClientRect().height;
		function a(e) {
	...
	function Rn(e) {
		...
		let n = t.left + t.width / 2, r = t.top + t.height / 2, i = Math.hypot(e.clientX - n, e.clientY - r), a = G.scale;
		if (i < 1) return;
		function o(e) {
	...
	function zn(e) {
		ze(e), R(e <= 0), y.current?.setVolume(e);
	}
	function Bn() {
		let e = !L;
		R(e), y.current?.setVolume(e ? 0 : z);
	...
	let Vn = [
		["源", r?.kind === "file" ? "本地文件" : r ? nt(i) : "未加载"],
		["状态", K.stalled ? "缓冲中" : Ie],
		["HTTP", String(j?.status || "--")],
		["CORS", j?.cors === "ok" ? "允许" : j?.cors === "blocked" ? "阻断" : "未知"],
		["Range", j?.acceptsRanges ? "206 Partial Content" : "完整响应 / 不支持 206"],
		["视频", nn[0] ? Ee(nn[0]) : "未识别"],
		["音频", rn[0] ? Ee(rn[0]) : "未识别"],
		["字幕", `${an.length} 条（${on.length} 条可用）`],
		["缓冲", `${K.bufferedAhead.toFixed(1)} 秒 · ${it(K.bufferedBytes)}`],
		["丢帧", String(K.droppedFrames)],
		["解码器", Pt]
	], Hn = Y || 100, Un = rt(Math.min(B, Hn) / Hn * 100), Wn = rt(Math.min(Math.max(K.bufferedEnd, B), Hn) / Hn * 100);
	return /* @__PURE__ */ d("div", {
		className: `${f ? "mx-player-embed" : "player-page"} ${Et ? "is-theater" : ""} ${se || ""}`.trim(),
		style: ce,
				...
				children: [/* @__PURE__ */ d("div", {
					...
					children: [
						...
						!A && !N && /* @__PURE__ */ d("div", {
							...
							children: [/* @__PURE__ */ u("span", { className: "spinner" }), /* @__PURE__ */ u("strong", { children: Ie })]
						}),
						A && !N && K.stalled && /* @__PURE__ */ d("div", {
						...
						N && /* @__PURE__ */ d("div", {
							...
							children: [
								...
								r && /* @__PURE__ */ d("button", {
									...
									onClick: () => Bt((e) => e + 1),
									children: [/* @__PURE__ */ u(ge, { size: 15 }), " 重新读取"]
								})
							]
						}),
						(pt.length > 0 || W) && /* @__PURE__ */ d("div", {
							className: `subtitle-overlay ${W ? "is-editing" : ""}`,
							style: {
								"--subtitle-font": ke(G.font),
								"--subtitle-scale": G.scale,
								"--subtitle-offset": `${G.offset}%`
							},
							"data-player-control": W ? "" : void 0,
							onPointerDown: W ? Ln : void 0,
							children: [pt.length > 0 ? pt.flatMap((e, t) => e.split("\n").map((e, n) => /* @__PURE__ */ u("span", { children: e }, `${t}-${n}-${e}`))) : /* @__PURE__ */ u("span", {
								className: "subtitle-sample",
								children: We
							}), W && /* @__PURE__ */ d(e, { children: [
								/* @__PURE__ */ u("span", {
						...
						/* @__PURE__ */ d("div", {
							className: `player-controls ${St ? "is-visible" : ""}`,
							"data-player-control": !0,
							...
							children: [/* @__PURE__ */ d("div", {
								...
								children: [/* @__PURE__ */ d("div", {
									className: "seek-rail",
									"aria-hidden": "true",
									children: [/* @__PURE__ */ u("i", { className: "seek-buffered" }), /* @__PURE__ */ u("i", { className: "seek-played" })]
								}), /* @__PURE__ */ u("input", {
									...
									max: Y || 100,
									step: "0.1",
									value: Math.min(B, Y || 100),
									onChange: (e) => Z(Number(e.target.value)),
									"aria-label": "播放进度",
									"aria-valuetext": `${H(B)}，已缓冲至 ${H(K.bufferedEnd)}`
								})]
							}), /* @__PURE__ */ d("div", {
								...
								children: [/* @__PURE__ */ d("div", {
									...
									children: [
										/* @__PURE__ */ u("button", {
											...
											title: X ? "字幕菜单打开时已暂停" : F ? "暂停" : "播放",
											"aria-label": F ? "暂停" : "播放",
											disabled: X,
											onClick: hn,
											children: F ? /* @__PURE__ */ u(fe, { size: 21 }) : /* @__PURE__ */ u(me, {
												size: 21,
												fill: "currentColor"
											})
										}),
										te && /* @__PURE__ */ u("button", {
											className: "control-button",
											title: "下一集",
											"aria-label": "下一集",
											onClick: te,
											children: /* @__PURE__ */ u(be, { size: 20 })
										}),
										/* @__PURE__ */ u("button", {
											className: "control-button",
											title: L ? "取消静音" : "静音",
										...
										/* @__PURE__ */ u("input", {
											...
											step: "0.01",
											value: L ? 0 : z,
											onChange: (e) => zn(Number(e.target.value)),
											"aria-label": "音量"
										}),
										/* @__PURE__ */ d("span", {
										...
										g && /* @__PURE__ */ u("button", {
											...
											"aria-pressed": Lt,
											onClick: bn,
											children: /* @__PURE__ */ u(ue, { size: 20 })
										}),
										g?.onCompose && /* @__PURE__ */ u("button", {
											className: "control-button",
											title: "发送弹幕",
											"aria-label": "发送弹幕",
											onClick: g.onCompose,
											children: /* @__PURE__ */ u(ve, { size: 19 })
										})
									]
								}), /* @__PURE__ */ d("div", {
									className: "player-control-group secondary",
									children: [
										on.length > 0 && /* @__PURE__ */ u("button", {
											className: `control-button ${dt ? "is-active" : ""}`,
											title: sn ? `字幕：${tt(sn)}` : "字幕",
											"aria-label": "字幕",
											"aria-pressed": dt,
											onClick: Nn,
										...
										/* @__PURE__ */ u("button", {
											...
											"aria-pressed": Et,
											onClick: yn,
											children: /* @__PURE__ */ u(he, { size: 20 })
										}),
										/* @__PURE__ */ u("button", {
											...
											children: /* @__PURE__ */ u(ye, { size: 20 })
										}),
										/* @__PURE__ */ u("button", {
											className: "control-button",
											title: wt ? "退出全屏" : "全屏",
											"aria-label": wt ? "退出全屏" : "全屏",
											onClick: _n,
											children: u(wt ? de : le, { size: 20 })
										})
									]
								})]
							})]
						...
						ht && /* @__PURE__ */ u($e, {
							rate: Be,
							setRate: (e) => {
								Ve(e), y.current?.setPlaybackRate(e);
							},
							audioTracks: rn,
							subtitleTracks: on,
							audioTrackId: st,
					...
					children: [
						/* @__PURE__ */ u("span", { children: K.stalled ? "缓冲中…" : Pt }),
						/* @__PURE__ */ d("span", { children: [
							"已缓冲 ",
							K.bufferedAhead.toFixed(1),
							" 秒"
						] }),
						/* @__PURE__ */ d("span", { children: ["当前时间 ", H(B)] })
					]
				})]
			})
		})]
...
function Je({ x: e, y: t, onClose: n, onStats: r, onAbout: i }) {
	let o = c(null);
	a(() => {
	...
	return /* @__PURE__ */ d("div", {
		...
		children: [
			/* @__PURE__ */ d("button", {
				role: "menuitem",
				onClick: r,
				children: [/* @__PURE__ */ u(se, { size: 15 }), " 播放器统计"]
			}),
			/* @__PURE__ */ u("span", { className: "menu-separator" }),
			/* @__PURE__ */ d("button", {
				role: "menuitem",
				onClick: i,
				children: [/* @__PURE__ */ u(_, { size: 15 }), " 关于 MX Player Pro"]
			})
		]
	});
}
function Ye({ page: t, tracks: n, selectedId: r, enabled: i, style: a, onSelect: o, onFontChange: s, onPage: c, onEdit: l }) {
	let f = t === "font", p = n.length + 1, ee = p * V + (p - 1) * Ge;
	return /* @__PURE__ */ d("div", {
		...
		children: [/* @__PURE__ */ d("div", {
			...
			style: { "--menu-body-height": `${ee}px` },
			children: f ? x.map((e) => /* @__PURE__ */ d("button", {
				className: `subtitle-font-item ${a.font === e.id ? "is-selected" : ""}`,
				onClick: () => s(e.id),
				children: [/* @__PURE__ */ d("span", {
					className: "subtitle-font-name",
					children: [e.label, a.font === e.id ? /* @__PURE__ */ u(ce, { size: 13 }) : null]
				}), /* @__PURE__ */ u("span", {
...
function Xe({ style: e, onReset: t, onDone: n }) {
	return /* @__PURE__ */ d("div", {
		...
		children: [
			...
			/* @__PURE__ */ d("button", {
				onClick: t,
				children: [/* @__PURE__ */ u(_e, { size: 13 }), " 恢复默认"]
			}),
			/* @__PURE__ */ d("button", {
				onClick: n,
				children: [/* @__PURE__ */ u(ce, { size: 14 }), " 完成"]
			})
		]
	});
}
function Ze({ rows: t, onClose: n }) {
...
function Qe({ onClose: e }) {
	return /* @__PURE__ */ d("section", {
		...
		children: [
			...
			/* @__PURE__ */ d("span", { children: ["v", B] }),
			/* @__PURE__ */ u("p", { children: "纯客户端 Matroska 播放器。文件和链接只在本机读取，视频帧由 WebCodecs 输出。" })
		]
	});
}
function $e({ rate: e, setRate: t, audioTracks: n, subtitleTracks: r, audioTrackId: i, subtitleTrackId: a, selectTrack: o, qualities: s, selectedQuality: c, onQualityChange: l }) {
	return /* @__PURE__ */ d("div", {
...
var st = r(function(e, t) {
	...
	return /* @__PURE__ */ u("div", {
		...
		onDrop: e.localPlayback ? (e) => {
			e.preventDefault(), i(!1), ee(e.dataTransfer.files[0]);
		} : void 0,
...
//#endregion
export { st as MXPlayerReact, st as default };
